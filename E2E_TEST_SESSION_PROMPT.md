...
- Guardian registration/rejection/expiry, adult A/B/C creation and disposable test-admin creation have already been exercised and ledgered. The guardian happy-path remains blocked by TR-185 because approval returns 500 and no approved-minor user is created. Activity Rewards (`FE-001` through `FE-004`, `DB-001` through `DB-003`), admin frontend API rows (`FE-005` through `FE-061`), auth frontend API rows (`FE-062` through `FE-074`) and concrete blog evidence rows (`FE-075`, `FE-077`, `FE-087`) have been transcribed in `E2E_COVERAGE_LEDGER.md` CP-003 through CP-007. Current recommended next work is `RUN-092`: continue coverage transcription/runtime testing from `FE-076` / `DB-004`, or resume the remaining control inventory from `CTRL-UI-001-000` / `UI-001`, unless the owner explicitly switches priority to fixing defects.
