...
| GET | /contracts/{id} | public (token optional) | id | Contract detail. Returns flat object: all contract fields + `giver_name`, `taker_name`, `applications[]` (each with `member_id`, `member_name`, `message`, `proposed_price_points`, `proposed_completion_date`, `status`, `giver_message_count`, `giver_requested_detail`, `detailed_proposal`, `has_rated`), `applicant_count`. `has_rated` is `true` when the authenticated caller has already submitted a rating for that application (requires valid Bearer token; always `false` when unauthenticated). Lazy-expires if listed and past deadline. |
...
| POST | /contracts/applications/{app_id}/rate | member | app_id, stars (1–5), testimony? | Submit a rating and optional testimony for the counterparty on a completed application. **Giver rates Taker; Taker rates Giver.** Guards: party check (must be Giver or the specific Applicant), `application.status == "completed"`, stars 1–5, one rating per party per application (btree|unique enforced). Role auto-determined: `as_giver` if caller is Giver, `as_taker` otherwise. Emits `contract_rated` notification to ratee. Response: the new `contract_ratings` row. |
