...
| GET | /me/activity | member | page?=1, per_page?=25 | Paginated transaction history. Returns `{items, itemsTotal, curPage, nextPage}`. Each item: `currency`, `side`, `amount`, `balance_after`, `description`, `ref_type`, `blog_id?`, `blog_title?`, `contract_id?`, `contract_title?`, `counterparty_name?`, `counterparty_code?`. For `contract_payment_sent/received` ref_types, description = "Contract payment sent/received — [title]" and counterparty fields populated (Taker for sent, Giver for received). |
...
| Method | Path | Auth | Key Inputs | Notes |
|---|---|---|---|---|
| GET | /contracts | public | contract_type? (vgc_administrated\|non_vgc), status?, sector?, page?, per_page? | List contracts. Response: `{items, curPage, nextPage}`. Each item enriched with `giver_name`, `giver_rating_count`, `giver_total_stars`, `applications_closed`. Lazy-expires listed contracts past `application_deadline` (status → `"expired"`, no wallet action — per-appointment escrow is handled by admin if assigned takers exist). (TR-118, TR-121, TR-122) |
| GET | /contracts/me | member | — | Member's personal contract dashboard. Response: `{as_giver[], as_taker[], as_candidate[]}`. `as_giver`: all contracts this member posted (all statuses), each enriched with `pending_applicant_count`, `assigned_count`, `work_submitted_count`, `disputed_count`, `applications_closed` (TR-121 — used by FE to group cards). `as_taker`: enriched with `giver_name`, `agreed_price_points`. `as_candidate`: enriched with `giver_name`, `proposed_price_points`, `giver_requested_detail`. All lists newest-first. |
| POST | /contracts | member | title, requirements, budget_points, contract_type (vgc_administrated\|non_vgc), sector, application_deadline, requested_completion_date, conditions? (required for VGC), notes? | Create contract. **No upfront wallet deduction for either type.** Escrow + 5% listing fee charged at appointment time (Secure only). Cap: Giver ≤10 active VGC contracts. (TR-122) |
| GET | /contracts/{id} | member | id | Contract detail. Returns flat object: all contract fields + `giver_name`, `taker_name`, `applications_closed`, `applications[]` (each with `member_id`, `member_name`, `message`, `proposed_price_points`, `proposed_completion_date`, `status`, `giver_message_count`, `giver_requested_detail`, `detailed_proposal`, `has_rated`, `applicant_rating_count`, `applicant_total_stars`), `applicant_count`. (TR-115, TR-119, TR-121) |
| PATCH | /contracts/{id} | member | id, title?, requirements?, budget_points?, deadline_days? | Update contract |
| POST | /contracts/{id}/apply | member | id, application_text, proposed_price_points? (int), proposed_completion_date? (date) | Apply for contract. Blocked when `applications_closed == true` (TR-121). |
| POST | /contracts/{id}/close-applications | member | id | **Giver-only.** Stops new applications on a listed contract without cancelling it. Sets `applications_closed = true`. Existing takers and pending applications are unaffected. Guard: `contract.status == "listed"`, caller must be Giver. Response: `{success: true}`. (TR-121) |
...
| POST | /contracts/{id}/cancel | member | id, reason? | **Giver withdrawal / cancellation.** Listed contract (no assigned takers): marks `cancelled`, no wallet action (no posting-time escrow). Listed with assigned takers: **rejected** — admin must resolve. Active contract (post-assign): marks `cancelled`, notifies other party via `contract_disputed`; admin applies any penalties. Response: `{status: "cancelled"}`. (TR-122) |
| GET | /contracts/applications/{app_id}/messages | member | app_id | Fetch private chat thread for an application. Only the Giver and the specific Applicant have access. Response: `{messages[], is_read_only, other_member_name, contract_title, application_status}`. Each message: `{id, sender_member_id, sender_name, message_text, created_at}`. `is_read_only=true` only when `status == "completed"` or `"disputed"` (TR-112: chat stays open during `assigned` and `work_submitted`). |
| POST | /contracts/applications/{app_id}/messages | member | app_id, message_text | Send a message in the private chat thread. Blocked when `application.status == "completed"` or `"disputed"`. Chat remains open during `pending`, `assigned`, and `work_submitted` (TR-112). Emits `contract_message` notification to the other party. |
...
| POST | /contracts/applications/{app_id}/appoint | member | app_id | Giver appoints a candidate as a Taker. Guards: Giver-only, `contract.status == "listed"`, `application.status == "pending"`, `giver_requested_detail == true`, `detailed_proposal != null`, candidate active-assignment count < 2. **Secure (`vgc_administrated`):** computes `listing_fee = agreed_price × 0.05`, `total_debit = agreed_price + listing_fee`; balance precondition; `db.transaction` debits Giver escrow + fee, credits admin wallet total; stores `agreed_price` to `application.escrow_amount_points`. **Independent (`non_vgc`):** balance precondition only (payment at verify-completion). Marks application `assigned`. Contract stays `"listed"` — other applications remain pending; Giver may appoint additional Takers. Emits `contract_assigned` notification. Response: `{success: true}`. (TR-122) |
...
| POST | /contracts/applications/{app_id}/verify-completion | member | app_id | **Giver-only.** Verifies the Taker's submitted work and releases payment. Guards: `application.status == "work_submitted"`, `agreed_price > 0`. **Secure (`vgc_administrated`):** `release_amt = application.escrow_amount_points ?? agreed_price`; `db.transaction` debits admin wallet → credits Taker wallet (`ref_type: "contract_payment_sent/received"`, `operation_type: "contract_payment"`). Giver is NOT debited again (already paid at appointment). **Independent (`non_vgc`):** checks Giver balance ≥ `agreed_price`; `db.transaction` debits Giver → credits Taker directly. Both branches set status → `"completed"` and emit `contract_completed` notification to Taker. Response: `{success: true, amount_transferred: <points>}`. (TR-122) |
...
| POST | /contracts/applications/{app_id}/rate | member | app_id, stars (1–5), testimony? | Submit a rating and optional testimony for the counterparty on a completed application. **Giver rates Taker; Taker rates Giver.** Guards: party check, `application.status == "completed"`, stars 1–5, one rating per party (btree|unique). Role auto-determined. Emits `contract_rated` notification to ratee. **Rating reward (TR-117):** after saving the rating, computes `reward = min(20, floor(agreed_price × 2 / 100))` where `agreed_price = proposed_price_points ?? budget_points ?? 0`. If `reward > 1`: debits admin VGC Points wallet and credits rater's VGC Points wallet (`operation_type: "constitutional_points"`, `ref_type: "contract_rating_reward"`). Response: `{rating: <contract_ratings row>, reward_points: <int>}` (`reward_points = 0` when threshold not met). |
| GET | /members/{member_id}/reputation | public | member_id | Public reputation profile for a member. Returns `{member_id, member_name, member_code, rating_count, total_stars, ratings[]}`. Each rating entry: `{id, stars, testimony, role, rater_name, rater_member_id, contract_title, created_at}`. Average = `total_stars / rating_count` (computed on FE to avoid int-division). Sorted newest-first. |
