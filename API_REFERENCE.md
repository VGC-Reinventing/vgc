...
| GET | /contracts/{id} | member | id | Contract detail. Returns flat object: all contract fields + `giver_name`, `taker_name`, `applications[]` (each with `member_id`, `member_name`, `message`, `proposed_price_points`, `proposed_completion_date`, `status`, `giver_message_count`, `giver_requested_detail`, `detailed_proposal`, `has_rated`), `applicant_count`. `has_rated` is `true` when the authenticated caller has already rated that application. Lazy-expires if listed and past deadline. (TR-115: changed from public to auth=user.) |
