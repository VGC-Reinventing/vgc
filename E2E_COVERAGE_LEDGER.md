...
| WF-05 | Token surrender | VGC75 | **Fixed (boundary-verified)** | Correctly-shaped creation requests were completely blocked by three stacked bugs: TR-263 (FE field-name mismatch), TR-265 (backend wallet-casing), and TR-266 (rate preview/computation from nonexistent config keys, which would have permanently re-blocked submission after TR-263's fix) — all three now fixed; the request reaches the real business-logic precondition and the browser shows a correct ₹85.00-for-10-tokens preview. Not exercised to a real created record (VGC75 has 0 tokens, and this session doesn't execute real-value writes regardless). TR-264 (UPI persistence gap) remains open. | **TR-263 ✓, TR-265 ✓, TR-266 ✓**, TR-264 (open) |
...
| Token surrender (WF-05) | `POST /token-surrender/create` | Correct-shape request from a real (0-balance) member | **Found and fixed three stacked defects blocking this feature end-to-end**: TR-263 (FE field-name mismatch), TR-265 (backend wallet-casing bug), and TR-266 (rate-preview computed from nonexistent config keys — found by re-testing TR-263's fix live in the browser rather than trusting the API-only check, and would have permanently blocked submission once TR-263 wired the broken rate into the request body). All three fixed; the corrected request now reaches the real "insufficient balance" precondition (VGC75 has 0 tokens, so this can never succeed) and the browser shows a correct ₹85.00 preview for 10 tokens (matching the SRS's ₹8.50/token exactly). | **TR-263 ✓, TR-264 (open), TR-265 ✓, TR-266 ✓** |
