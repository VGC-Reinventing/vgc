...
| WF-04 | INR declaration → token purchase | VGC75, real admin (VGC53) | **Pass — admin-side confirmed with a real 2FA session** | `/wallet/declarations` renders the real "Donation ₹500 · Pending" record. **Acquired a genuine, fresh admin 2FA session** (existing test-admin account, real password-reset + real emailed OTP flow via Gmail, no backend changes) and used it to: reject a real pending declaration end-to-end (`declaration.status → "rejected"`, `rejected_by` correctly attributed) — a fully safe, non-wallet-touching admin action; and confirm `declarations/{id}/verify`'s validation boundary with the real admin session (`404` for a nonexistent id, `"Already processed"` for an already-decided one). The final credit-issuing branch of `verify` was not completed (it directly mutates a real wallet — same boundary held all session, now proven to hold even with genuine admin credentials, not just a 403). Also confirmed the real `/admin/declarations` UI renders correctly with a real admin session (never tested with genuine admin auth before) — found and fixed **TR-272** (a stale button/badge state on the surrender panel; see WF-05). | Confirms TR-189, TR-196, TR-176 (2FA gate itself proven to work end-to-end) |
...
| WF-15 | Loan lifecycle | VGC75, disposable member (id 84, cleaned up), real admin (VGC53) | **Pass — admin-side confirmed with a real 2FA session** | TR-260 fixed: `getMyLoans` unwraps `.loans`; the "My Loans" tab now renders the real loan (₹2,000, Pending) in the browser. Repay button correctly gated to `status == "active"` with `outstanding_tokens > 0`. **Admin side, with a genuine fresh 2FA session** (TR-213's `enforce_role` gate proven end-to-end, not just via a 403): created a disposable loan request and **rejected it for real** (`admin/loans/{id}/reject` — safe, no wallet mutation, fully completed and verified `status: "rejected"`); confirmed `admin/loans/{id}/approve`'s validation boundary (`404` for a nonexistent loan, `"Only pending loans can be approved"` for the just-rejected one) — the actual disbursement branch (`mutate_wallet`/`mutate_wallet_unchecked` debiting/crediting real wallets) was not completed, same boundary held with real admin credentials. Fixture cleaned up (member, wallets, loan, notification all deleted, verified). | TR-201 (form ✓), **TR-260 ✓, TR-213 ✓ (real admin-session proof)** |
| WF-05 | Token surrender | VGC75, real admin (VGC53) | **Fixed & admin-side confirmed with a real 2FA session** | Correctly-shaped creation requests were completely blocked by three stacked bugs: TR-263 (FE field-name mismatch), TR-265 (backend wallet-casing), and TR-266 (rate preview/computation from nonexistent config keys) — all fixed; request reaches the real business-logic precondition, browser shows a correct ₹85.00-for-10-tokens preview. **TR-264** (UPI persistence gap) also fixed. **Admin side, with a genuine fresh 2FA session**: confirmed `token-surrenders/{id}/complete`'s validation boundary against real existing surrender rows (`404` for a nonexistent id, `"Only pending surrender requests can be completed"` for an already-rejected one, correct real path `EOOlx4pf/token-surrenders/{id}/complete` — not `admin/token_surrenders/...` as the file path structure suggested) — the actual debit branch not completed (real wallet mutation). Found and fixed **TR-272** live in the real admin UI: the surrender panel showed "Complete Surrender" on already-rejected rows (only excluded `completed`, not `rejected`) and used the same violet badge tone for rejected as pending — both only caught because this screen was finally rendered with genuine admin auth. Not exercised to a real created surrender record (VGC75 has 0 tokens). | **TR-263 ✓, TR-264 ✓, TR-265 ✓, TR-266 ✓, TR-272 ✓** |
