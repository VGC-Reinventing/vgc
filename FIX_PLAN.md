...
**Fix approach:**
- `login_POST.xs`: add explicit `suspended == false` and `email_verified_at != 0` preconditions before token issuance, both returning one generic rejection message (do not leak *which* condition failed — that itself would be a smaller version of TR-163/186's enumeration bug).
...
| TR-226 | Xano file upload is wired but the live instance plan doesn't support `storage.create_attachment` — this may be a plan/billing decision, not a code fix; confirm with the owner before spending engineering time on a workaround. | §17.6 |
