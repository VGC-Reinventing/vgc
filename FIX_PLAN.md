...
**Status: Clusters F2a and F2b closed 2026-07-21 (12 TRs); Cluster F2c not yet started.** TR-181, TR-190, TR-191, TR-204, TR-205, TR-206, TR-227, TR-232, TR-240 all fixed — see their Resolved entries in `TEST_REGISTER.md`. Two previously-uncataloged instances of the wallet-casing family were found and fixed in the same pass (`declarations/{id}/verify`, `token-surrenders/{id}/complete`) — 7 total instances now, all sharing one function. A new, previously-uncataloged defect was found and fixed while verifying TR-181 (`PATCH /profile` leaked the password hash on every save — logged as TR-252). A real XanoScript limitation was discovered and documented: a custom function calling another custom function via `function.run` fails at runtime, even though the identical call works fine from an API endpoint — no shipped fix is affected, but keep this in mind for Cluster F2c and beyond. **Known gap**: `admin/wallets/adjust`'s admin-success path (and the standing `VGC53` wallet-residue correction it was meant to enable) could not be independently live-tested — creating a fresh 2FA-verified admin fixture was blocked twice by the tooling safety classifier. Do this the next time a real admin session is naturally available. Cluster F2c (log_admin_action + PTS transaction safety, TR-208/TR-209/TR-237) is next.
