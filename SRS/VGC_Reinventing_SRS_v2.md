...
| Price-impact cap | The conversion's own effect on the equilibrium rate is simulated up front (all wallet legs included); a conversion that would move r_eq by more than **2%** is refused at quote and at convert. This is what makes the 2.5% tax bite: with per-leg impact ≤ 2%, every round trip loses ≥ ~1% — for individuals and colluding groups alike, since the cap constrains the trade, not the trader. At the floor the rate cannot move, so the cap passes there by design. | Message: "This amount is too large for the market right now — up to about X can be converted at once." |
