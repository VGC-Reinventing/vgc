...
| 2.37 | 2026-09-27 | §11.7/§11.13 (owner): **candidates' funding is held in escrow from VGC Admin's confirmation** — Independent: the points budget; Secure: tokens worth 2× the budget at that moment's rate, tax included (confirmation is refused if the candidate cannot cover it). Held in the VGC Admin wallet, counted as the candidate's in the PTS formula. The moment a game's pioneer is decided (selection, election close or tie-break) the winner's escrow **funds the season at once** — it locks and lists with no further step — and **every other candidate's escrow is refunded**; decline and reopen refund too. This supersedes 'never at candidacy' (§11.13.1), the winner's 12-hour funding window and the runner-up cascade (§11.7.2), which no longer arise. Fixed at the same time: a tie-broken election never created its season; election close treated every tie as won by 'candidate 0' (an omitted number is 0, not null); deposit refunds on close and decline paid out fees that had never been paid. |
| 2.36 | 2026-09-27 | §11.13.1 (owner): Independent ticket revenue defaults to **90% pioneer / 10% VGC Admin** — the pioneer may still propose a different split, which VGC Admin confirms after the proposal chat. The leftover-points withdrawal is built: after the season ends and before it is settled, the pioneer may withdraw **any amount up to the game wallet's full balance**, including points inherited from earlier seasons (owner decision); after settlement whatever remains stays in the game's pool. |
| 2.35 | 2026-09-27 | §11.13.2/§11.15 built (owner decisions): every event on a candidacy states a **points budget (cap)** and at least one way to win points — placements, a flat participation reward, named prizes, and (alongside them) **proof-of-work tasks** paid a fixed amount per approved proof (description and/or up to 4 photos, reviewed by the pioneer or VGC Admin, like a marketplace listing but in points). The events' caps together may not exceed the season budget (checked at confirmation). Points leave the game wallet only through these rules, each award ledgered. The season snapshots the game wallet when it opens; the **80% test is against that opening balance** (inherited pool included); the Secure reward is **26% of the points actually distributed**, paid from the escrow, and the rest of the escrow — all of it when the test fails — goes to the game's pool. The candidacy form shows the real Secure deposit (2× budget at the live rate, tax included). The pioneer's free-typed distribution log and the retired deposit action are gone. |
| 2.34 | 2026-09-27 | §3.4.1 extended (owner): **tokens paid into escrow are counted as the paying member's** in the PTS formula until the transaction settles, so paying never moves the rate. Marketplace and course orders were the gap — their held tokens were counted as nobody's; they now count as the buyer's until the order is settled, cancelled or refunded (points-priced orders likewise on the points side). Once the time runs out the escrow must be credited per the split: period-pass expiry and post-delivery auto-settlement now run for **all** orders whenever the rate or an order list is read, not only when one order is opened — tuition passes #10–#12 (Usha → Vishal, expired 19 Sep, windows closed 22 Sep) had never settled for that reason and settled 27 Sep at 76 / 4 tokens each. Course session-cancellation refunds are recorded against the order, and a teacher payout pays only on tokens still held, then settles the orders. |
| 2.33 | 2026-09-26 | §3.4.1 added (owner): **tokens and points are never destroyed once minted** — every debit lands in some wallet. Token Surrender now moves the surrendered tokens to the VGC Admin token wallet (supersedes 2.32's "removed from circulation"; under the current §4.1 formula each surrendered token raises `D` by ₹10). Pioneer candidacy fees/deposits and the Secure funding stake are now **held in the VGC Admin token wallet** as escrow (counted as the member's in the PTS formula while undecided, including during an election) instead of being credited to nobody; a forfeited election deposit simply stays with Admin. The admin wallet-adjustment tool now ledgers every change, moves a negative token/points adjustment to VGC Admin, and refuses to reduce Admin's own tokens or points. |
| 2.32 | 2026-09-26 | §3.7.1 extended (owner): **token surrender margin**. A token is minted at the purchase rate (`inr_per_token`, ₹10) and bought back at the surrender rate (₹8.5). On surrender completion the PTS Pool is debited the **full purchase value**, tokens × purchase rate (₹10/token): the member's payout (₹8.5/token, PTS Pool first, Spendable on shortfall as before) plus the spread (₹1.5/token), which **moves to the Spendable Pool** so it can fund expenses. Surrendered tokens are removed from circulation, not credited to VGC Admin, so a surrender exactly reverses its token's purchase and leaves the rate's `D` unchanged; the pools stay equal to real cash. The spread follows the configured rates, and only what the PTS Pool still holds after the payout can move. |
| 2.31 | 2026-09-25 | §11.13.4 added (owner): **season participation tickets**. One mandatory season ticket admits to every event; each event may add up to 4 event-only tickets. Independent: the pioneer proposes them and their % share of ticket revenue on the candidacy; Secure: VGC Admin sets them and keeps 100%. New gate **Admin confirms** after the proposal chat — locks every candidacy field, tickets and share included; funding is refused until confirmed (visible → decided → confirmed → funded → listed). At funding the system creates the tickets as marketplace listings (RG-ticket pattern: 1 per member, settled at purchase, no cart, no member edits). Listings are on sale only from season start to season end, and seasons now start and end automatically (lazy). Event entry requires the season ticket or one of that event's tickets. Every sale credits the VGC Admin token wallet at once (Admin-side in PTS); Independent revenue is escrow until **Admin marks the season settled**, which pays the pioneer's share and returns the Secure stake. The 26% reward and game point pool at settlement remain to be built. Also fixed: the Secure stake was subtracted from the Admin side of the PTS formula although it was never credited there. |
...
| 2.24 | 2026-09-18 | §17 investment-payout weight corrected (owner): investment inflows count 0.1 into D (90% is a liability, not backing), but paybacks left through I_expense at full weight — subtracting ~90% that D never held, so a completed investment netted large phantom damage (a ₹10,900 investment: +1,090 in, −11,772 out). Investment-payout expenses (auto-written, `specific_category = "Investment Payout"`) are now excluded from I_expense and counted at **0.2 × payout**, which approximates the exact reversal (0.1·principal + interest) for both the 10% Growth and 8% Income products within ~2%. Lifetime effect of a completed investment on D is now ≈ −interest-paid, as it should be. Same day: token-surrender completion auto-writes its Platform Outflow (the last manual payout type), and at owner request the platform's financial state was reset to a true zero — all wallets, ledgers, declarations, orders and conversions cleared; content (blogs, listings, groups, accounts, contracts) retained. |
| 2.23 | 2026-09-12 | Close-out of the marketplace cycle, from live testing with the first real vendor. §6.3: proposal **decisions are surfaced** — accept/request-changes/reject each posts an admin message into the proposal chat and notifies the proposer (previously silent), and a sent-back proposal (changes requested **or rejected**) follows the blog-review pattern: editable, and saving **resubmits** it into the admin queue with an admin notification; the Marketplace gains a "My proposals" entry (the list was buried under the propose form). §6.8.1: a **period pass's validity window is stated by the seller at confirmation** (valid-from/valid-until, e.g. "1st to 30th September") — the earlier per-unit×quantity formula stamped a 4-subject September order "Active until 8 Jan"; the listing's pass-days field is advertisement only. Admin dispute handling: the disputed-order card is the full cockpit (buyer's reason, POD evidence, promise fields, both chats) and the **resolution form is inline on the card** — no order-ID entry. Fixes: item_type made optional (its required-check blocked every standard proposal after the dropdown's retirement); the own-item purchase guard resolves the real vendor. |
| 2.22 | 2026-09-11 | §6.8.1: the **delivery-medium axis** (Digital Good / Physical Good / In-Person / Online Live / Remote Deliverable / Platform-Hosted) joins the nature taxonomy — declared at proposal, copied to the listing, shown at checkout ("Reaches you as…") and steering order-screen wording ("Mark sent to buyer" for digital, meeting-link prompt for online live). The propose form's legacy free-text Item Type dropdown is retired; nature and medium are chosen from definition cards. |
| 2.21 | 2026-09-11 | §6.8.1 gains the **item-nature taxonomy**: Product, Service Job, Period Pass, Session Ticket, Use Pack, Lifetime Access, Rental — declared at proposal with per-nature config (pass validity days, uses per unit, rental days), shown at checkout. Period passes activate at confirmation and settle automatically after expiry + response window (lazy, on read); use packs are drawn down one vendor-recorded use at a time, the last use starting the response window through an automatic POD; ongoing natures are disputable at any time while active and lose free self-cancel at activation. Rentals stamp a return-due date at delivery. Courses and RG blog tickets are the pre-existing Session Ticket and Lifetime Access implementations, unchanged. |
| 2.20 | 2026-09-11 | §6.8.1 extended with the **delivery commitment**: the proposer states a binding delivery period (1–90 days) at proposal, admin-approved and changed only via an edit request; each order stamps its deadline (placed + period), the confirm-time promise may not exceed it, and the buyer sees it at checkout and on the order. **VGC Admin can be involved only at whichever comes first** — the commitment running out before delivery, or delivery being confirmed (then within the buyer-response window, now **3 days**, after which the order auto-settles; the buyer can expedite by marking settled). The declined-cancellation shortcut to Admin is removed. Admin resolutions gain **resume_order** (both parties agree to wait; the buyer may re-raise) and partial splits now charge the admin fee **only on delivered-order disputes** — a late-undelivered split carries no fee. The **vendor may cancel unilaterally any time before delivery is confirmed**: full escrow to the buyer, nothing to the vendor, open pre-delivery disputes auto-resolve in the buyer's favour. Escrow continues to release only at settlement. |
...
| PTS Pool (Admin only) | Indian Rupees (INR) | No | The INR half of the Point Token Scheme's backing (§4.1's `D`). Credited by the weighted share of every verified Donation, Grant, Investment and Token Purchase, and by the net of a settled Sponsorship (§3.7.1). Debited by Token Surrenders at the full purchase rate (₹10/token: the ₹8.5 payout plus the ₹1.5 spread moved to the Spendable Pool), and by Investment Payout shortfalls the Spendable Pool cannot cover. **Never touched by Loan Disbursement.** |
| Spendable Pool (Admin only) | Indian Rupees (INR) | No | VGC Admin's discretionary operating cash — the pool ordinary Platform Outflow entries, and Loan Disbursements, draw from. Credited by the complementary share of Donation/Grant/Investment, and by Sponsorship settlement shortfalls the escrow doesn't cover. Also credited, on each completed Token Surrender, with the buy/sell spread moved out of the PTS Pool: tokens × (purchase rate − surrender rate), ₹1.5/token at today's rates (owner rule 2026-09-26). Debited by ordinary expenses, Loan Disbursements, Investment Payout installments, and by Token Surrender shortfalls the PTS Pool cannot cover. |
...
Member submits a VGC Token Surrender Request Form specifying the number of tokens and their UPI ID. VGC Admin reviews the request and sends the INR equivalent to the member's UPI ID at the current surrender rate. VGC Admin marks the request as Completed. Member's VGC Token Wallet is debited, the surrendered tokens are credited to the VGC Admin token wallet (§3.4.1), and Member's INR Receipt Ledger is credited. VGC Admin logs the outgoing INR payment as a Platform Outflow entry in the Expense Tracker.

#### 3.4.1 Conservation of Tokens and Points

> **Owner rule 2026-09-26 (SRS 2.33).** Once minted, a VGC Token or VGC Point is **never destroyed**. Every debit from a wallet is matched by a credit to another wallet, so the total supply only ever grows (by minting) and is never reduced.

| Flow | Where the tokens/points go |
| --- | --- |
| Token Surrender | Member → VGC Admin token wallet |
| PTS conversion (either direction) | Member's full gross → VGC Admin; payout from Admin's wallet (points minted only when Admin's points run short) |
| Pioneer candidacy fee / election deposit | Held in VGC Admin's token wallet as escrow; returned on decline or ≥3 votes, otherwise stays with Admin |
| Secure funding stake | Held in VGC Admin's token wallet as escrow; returned at season settlement |
| Marketplace / course order escrow | Held pending the order's outcome and **counted as the buyer's** in the PTS formula meanwhile (SRS 2.34); when the time runs out it is credited per the split to the seller and VGC Admin, or refunded to the buyer — never dropped |
| Admin wallet adjustment | A negative token/points adjustment moves to VGC Admin; Admin's own tokens/points cannot be reduced |
...
| Token Surrender completion (`token-surrenders/{id}/complete`) | Fund the payout from the PTS Pool first; on insufficient balance, draw the shortfall from the Spendable Pool. Final, no reconciliation. Additionally move the buy/sell spread, tokens × (purchase rate − surrender rate), from the PTS Pool to the Spendable Pool — so the PTS Pool gives up the full purchase value (owner rule 2026-09-26). |
...
**Item natures** (owner taxonomy 2026-09-11) — every listing declares what
kind of thing it sells, which decides its lifecycle:

| Nature | Pay | Access | Verified by | Lifecycle |
| --- | --- | --- | --- | --- |
| Product (default) | Once | One delivery | Proof of delivery | The four phases below |
| Service Job | Once | Until work done | Proof of completion | The four phases below |
| Period Pass | Per window | Seller states valid-from/until at confirmation (`pass_period_days` is the advertised typical duration; quantity = parallel units) | Vendor checks validity per visit | Confirm = activate with the stated window; expiry writes an automatic POD → response window → settle |
| Session Ticket | Per event | One check-in | Consumed at check-in | A Use Pack of exactly 1 |
| Use Pack | Once for N uses | Until quota exhausted | Vendor records each use | Last use writes an automatic POD → response window → settle |
| Lifetime Access | Once | Forever | Platform-enforced | Settles at purchase (RG blog tickets) |
| Rental | Once per term | `rental_period_days`, then returned | POD at handover; return date shown | Product flow + return-due date |

Each listing also declares its **delivery medium** — how the thing reaches
the buyer, orthogonal to the nature: **Digital Good** (a code/key/file),
**Physical Good** (shipped or handed over), **In-Person Service** (parties
meet), **Online Live Service** (over a call or stream — the confirm-time
note carries the meeting link), **Remote Deliverable** (work handed over
digitally), and **Platform-Hosted** (VGC itself enforces access — RG blogs
and courses). The medium changes the wording of dispatch and proof, not the
money flow. The propose form presents both axes as definition cards rather
than bare dropdowns.

Ongoing-access natures (Pass/Pack/Session) differ from the phase flow in two
ways: the buyer may involve VGC Admin **at any time while active** (denial of
access must not wait for a calendar deadline), and free self-cancel ends at
**activation** rather than at preparation — a day-29 cancel of a 30-day pass
would be theft of service. The vendor's unilateral full-refund cancel still
applies until completion.

Every listing carries a **delivery commitment** (1–90 days, set at proposal,
admin-approved, changed only via an edit request). Each order's deadline is
placed-at + that period; the buyer sees it at checkout and on the order, and
the phase-1 promise may not exceed it. **VGC Admin enters an order's life at
whichever comes first**: the commitment runs out before delivery (the buyer
may then raise a ticket — Admin mediates toward waiting longer, with the
buyer's agreement, or a fair escrow split with **no admin fee**), or delivery
is confirmed (the buyer has a **3-day response window** to dispute — here a
split deducts the admin fee first — or to settle early; silence auto-settles).
The vendor may cancel unilaterally at any time before delivery is confirmed:
the buyer receives the full escrow, the vendor receives nothing, and any open
pre-delivery dispute resolves in the buyer's favour automatically.
...
**The purchaser guarantee is status-proof** (owner rule, 2026-09-21): a member
who bought a Revenue Generator ticket while the blog was live keeps read
access through **every** later state the blog can reach — abandonment,
takedown, and the author archiving it. No action available to the author can
take away what was bought; the only hard delete in the system (§8.7 table,
row 1) is limited to never-published blogs, which by definition can have no
purchasers. Until 2026-09-21 archiving was the one path that violated this —
the archive endpoint promised purchaser access in a comment while the read
endpoint knew nothing of it, so archiving an RG blog silently cut every
ticket-holder off.
...
| Winner funds | Automatically: the winner's funding has been in escrow since VGC Admin confirmed their candidacy, so the season is funded the moment the result is decided, and every other candidate's escrow is refunded (SRS 2.37). Candidacies should therefore be confirmed before voting opens. |
...
| Winner fails to fund | No longer arises for a confirmed candidacy — its funding is already held (SRS 2.37). An unconfirmed winner's season waits in `pending_funding` and funds the moment VGC Admin confirms it. |
...
| Aspect | Rule |
| --- | --- |
| Source of points | The pioneer's own VGC Points: the full stated budget is held in escrow from VGC Admin's confirmation of the candidacy, moves to the Game Points Wallet the moment the candidate becomes the pioneer, and is refunded the moment another candidate does (SRS 2.37). |
| Participation ticket | Proposed by the **pioneer** as a marketplace item linked to the season — RG-blog-ticket pattern: members must hold it to enter the season's events. Revenue escrows until season settlement, then splits **VGC Admin / pioneer per the share the pioneer proposed and VGC Admin confirmed** — 90% pioneer / 10% VGC Admin by default (§11.13.4). |
| Leftover points | After the season ends and before it is marked settled, the pioneer may withdraw any amount of the points remaining in the Game Points Wallet, up to its full balance (including any pool inherited from earlier seasons — owner decision 2026-09-27). After settlement the remainder stays in the game's pool. |
...
| The 80% datum | 80% of the **game wallet's balance when the season opens** (the distributable half plus any inherited pool; owner decision 2026-09-27) — in the example with no inherited pool, 3,200 points — measured from the season ledger's rule-governed distributions (§11.15), never self-reported. |
...
#### 11.13.4 Season Participation Tickets

> **Added 2026-09-25 (owner, SRS 2.31).**

| Aspect | Rule |
| --- | --- |
| Season ticket | Exactly one per season, mandatory. Admits the holder to **every** event of the season. |
| Event tickets | Optional, **up to 4 per event**. Each admits to that event only. |
| Who defines them | Independent: the pioneer, on the candidacy, together with their **% share of ticket revenue**. Secure: VGC Admin, on the candidacy; 100% of revenue is Admin's. |
| Ticket fields | Name, description, price (VGC Tokens), stock, image, up to 8 buyer-information fields. |
| Confirmation | After the proposal chat, VGC Admin **confirms** the candidacy. Confirmation locks every field — season details, events, tickets and the revenue share. Funding is refused until confirmed. Admin may reopen an unfunded proposal; it must be confirmed again. For an election, candidacies are confirmed before voting so the winner's 12-hour funding window is not spent waiting on Admin. |
| Listing | Created by the system at funding as marketplace listings managed like Revenue Generator blog tickets: one per member, access granted (order settled) at purchase, not purchasable through the cart, not editable or deletable by members. |
| Sale window | On sale only from the season's start date-time to its end date-time. The season starts (events open) and ends (events close, tickets delist) automatically, evaluated lazily on read. |
| Entry rule | Entering an event requires holding the season ticket or one of that event's tickets. Seasons funded before tickets existed are exempt. |
| Money | Every sale credits the **VGC Admin token wallet** at once, so it counts Admin-side in the PTS formula. Independent: this is an escrow — nothing reaches the pioneer until settlement. |
| Settlement | When the season has ended, VGC Admin **marks it settled** (one-shot): Independent — the pioneer's confirmed share of all ticket revenue moves Admin → pioneer; Secure — the token stake is returned in full. The 26% reward and game point pool steps of §11.13.2 are not yet part of settlement. |
...
| Element | Rule |
| --- | --- |
| Structured reward rules | Each event on the candidacy declares an **allocation cap** (its slice of the season budget; the sum of caps must not exceed the budget) and one or more reward components: a **placement table** (1st: X pts, 2nd: Y, 3rd: Z, …), a **participation reward** (flat points per accepted entry, with an entry cap), **judged awards** (named prizes with fixed amounts), and **proof-of-work tasks** (owner decision 2026-09-27, alongside the others): a fixed payment per approved proof, with a limit on paid proofs and on proofs per member; a member submits a description and/or up to 4 photos and the pioneer or VGC Admin approves (paying automatically) or rejects with a reason. Locked with the season when VGC Admin confirms the proposal. |
