...
| 2.32 | 2026-09-26 | §3.7.1 extended (owner): **token surrender margin**. A token is minted at the purchase rate (`inr_per_token`, ₹10) and bought back at the surrender rate (₹8.5). On surrender completion the member's payout is debited from the PTS Pool exactly as before, and the spread — tokens × (purchase rate − surrender rate), ₹1.5/token today — is additionally **credited to the Spendable Pool** so it can fund expenses. The spread follows the configured rates rather than a fixed ₹1.5. |
